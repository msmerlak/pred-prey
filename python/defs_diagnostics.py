...
class Grid:
    ...
    def agents_move_and_age(self):
        for species in ['hare', 'wolf', 'human']:
            pop_size = len(self.population[species])
            if self.FC:
                delta = np.random.randint(self.size, size = (pop_size, 2))
            else:
    ...
    def plot(self):
            ...
            plt.subplot(515)
            x_lifetime, y_lifetime = zip(*sorted(normalized_dictionary(dict(self.lifetime['hare'])).items()))
            plt.xscale('log')
...
def normalized_dictionary(d, target=1.0):
   raw = sum(d.values())
   factor = target/raw
